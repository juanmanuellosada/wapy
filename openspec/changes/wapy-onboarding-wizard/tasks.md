...
- [x] 1.1 Add `react-dropzone` (drag&drop upload) and `@dnd-kit/core` + `@dnd-kit/sortable` (drag-reorder) to `package.json`. Install.
- [x] 1.2 Confirm `npm run build` still passes with new deps.

## 2. State + schemas + actions

- [x] 2.1 Create `lib/onboarding/schemas.ts` with zod per step: `basicsSchema`, `lookSchema` (accent_color enum of 6 hex), `sectionsSchema` (min 1), `productsSchema` (min 1, each with price_cents>=0, max 5 images), `whatsappSchema` (E.164 regex). Export inferred types.
- [x] 2.2 Create `lib/onboarding/palette.ts` exporting the 6 accent colors as a const array (e.g., `['#22c55e','#eab308','#ef4444','#3b82f6','#a855f7','#ec4899']` — final list TBD, pick from `ui-ux-pro-max` palettes for variety + accessibility).
- [x] 2.3 Create `lib/onboarding/state.ts`:
  ...
  - NOTE: step constants extracted to `lib/onboarding/steps.ts` to be client-safe (no server-only imports)
- [x] 2.4 Create `lib/onboarding/actions.ts` with `'use server'`:
  ...
  - `saveProducts` → per-product `saveProduct` action (immediate save per product, not batch). `advanceProductsStep` action to advance step 4.
...
- [x] 2.5 Create `lib/onboarding/storage.ts` with client-side helpers (Client Components import these):
...
- [x] 3.1 Replace `app/onboarding/page.tsx` (placeholder) with Server Component that calls `getStoreState`, redirects to `/onboarding/{currentStep}` (or `/dashboard` if published).
- [x] 3.2 Create `app/onboarding/[step]/page.tsx` Server Component:
...
- [x] 3.3 Update `proxy.ts` PROTECTED_PREFIXES to still cover `/onboarding/:path*` (already does). No changes needed, just verify.

## 4. Stepper component (`ui-ux-pro-max` style)

- [x] 4.1 Create `app/onboarding/components/Stepper.tsx`:
...
- [x] 5.5 `ProductModal.tsx` (Client, used inside StepProducts):
...
- [x] 6.1 Create `app/onboarding/components/ImageUpload.tsx` (Client):
...
- [x] 6.2 Create `app/onboarding/components/LogoUploader.tsx` (Client, wraps ImageUpload):
  - Specialization: single file, replaces, ties to `stores.logo_url`.
- [x] 6.3 ProductModal uses ImageUpload directly with maxCount=5.

## 7. Publish flow + /dashboard placeholder update

- [x] 7.1 Verify `publishStore()` server action covers all the spec scenarios.
- [x] 7.2 Update `app/dashboard/page.tsx` placeholder (NOTE: this file may not exist yet — `dashboard` was never a route in earlier Fases since middleware protects it but no page exists). Create if missing.
...
- [x] 8.1 `removeProduct` action: before DELETE row, call `supabase.storage.from('product-images').remove([...paths])` for each URL in `image_urls`. Log + continue if remove fails.
- [x] 8.2 `removeSection` action: rely on FK ON DELETE SET NULL for products. No storage cleanup needed (products' images survive).

## 9. Verification

- [x] 9.1 `npm run build` — must pass.
- [x] 9.2 `npm run dev` — verify routes compile:
  ...
  - `/onboarding/whatsapp`
  - `/onboarding/review`
- [x] 9.3 Zero TypeScript `any`. All store/product/section data typed via `lib/supabase/types.ts`.
...
- [x] 11.1 Commit 1: `Add onboarding state machine, schemas, and server actions` (lib/onboarding/, deps update).
- [x] 11.2 Commit 2: `Add onboarding wizard pages: stepper, basics, look, sections` (3.x routing + 4.x Stepper + 5.1-5.3 step components + image upload component for logo).
- [x] 11.3 Commit 3: `Add product CRUD with multi-image upload` (5.4 StepProducts + 5.5 ProductModal + ImageUpload extended).
- [x] 11.4 Commit 4: `Add whatsapp step, review summary, and publish flow + /dashboard placeholder` (5.6, 5.7, 7.x).
- [x] 11.5 Update `tasks.md` marking all completed; commit it.
