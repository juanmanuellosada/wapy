...
- [ ] 1.1 Create `lib/admin/schemas.ts` with zod schema `addEmailSchema` ({ email, grant_role: 'owner' | 'superadmin' }). Export the inferred TS type.
- [ ] 1.2 Create `lib/admin/actions.ts` with `'use server'`:
...
- [ ] 2.1 Replace `app/admin/page.tsx` placeholder with real panel:
  - Server Component.
...
- [ ] 2.2 Create `app/admin/AddEmailForm.tsx` (Client Component):
...
- [ ] 2.3 Create `app/admin/WhitelistTable.tsx` (Server Component, receives rows as prop):
...
- [ ] 2.4 Create `app/admin/RowActions.tsx` (Client Component):
...
- [ ] 3.3 Confirm TypeScript types from `lib/supabase/types.ts` are used (no `any` for whitelist rows).
...
- [ ] 5.1 Commit 1: `Add superadmin server actions and schemas for whitelist management` (`lib/admin/{schemas,actions}.ts`).
- [ ] 5.2 Commit 2: `Build /admin whitelist panel with add, re-invite, remove` (`app/admin/page.tsx` rewritten + 3 new components).
- [ ] 5.3 Commit 3: `Mark Phase 3 tasks complete` (`openspec/.../tasks.md`).
